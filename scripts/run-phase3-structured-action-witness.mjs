...
} from "./phase3-structured-action-detector.mjs";
...
const detectorFile = path.join(repoRoot, "scripts", "phase3-structured-action-detector.mjs");
const resultRoot = path.join(repoRoot, "results", "v2", "isolation", "phase3");

function parseArgs(argv) {
  if (argv.length === 1 && argv[0] === "--help") {
    console.log("Usage: node scripts/run-phase3-structured-action-witness.mjs --publish");
    process.exit(0);
  }
  if (argv.length !== 1 || argv[0] !== "--publish") {
    throw new Error("Phase 3 authority requires exactly --publish; exploratory provider runs are prohibited.");
  }
}
...
function cleanupVerified(parent, target) {
  if (!contained(parent, target)) throw new Error("Refusing cleanup outside the verified Phase 3 run root.");
...
function sanitize(text, replacements) {
  ...
  output = output.replaceAll("/home/phase3", "<CONTAINER_HOME>");
  output = output.replace(/\/workspace(?:\/[A-Za-z0-9._/-]+)?/g, "<CONTAINER_PATH>");
  output = output.replace(/(?:source_)?(?:thread|task|session)[_-]?id\s*[:=]\s*[0-9a-f-]{20,}/gi, "PRIVATE_ID_REDACTED");
...
function publicSafe(text) {
  ...
  const privateId = new RegExp("(?:source_)?(?:thread|task|session)[_-]?id\\s*[:=]\\s*[0-9a-f-]{20,}", "i");
  const rawPrivateField = new RegExp("(?:raw_command|item_id|session_id|thread_id|task_id|source_thread_id)", "i");
  const rawContainerPath = new RegExp("/(?:workspace|codex-home)(?:/|\\s|\")");
  return !windowsHome.test(text) && !unixHome.test(text) && !secret.test(text)
    && !privateId.test(text) && !rawPrivateField.test(text) && !rawContainerPath.test(text);
}
...
function inspectPreflight(containerId, contract, commandArgs) {
  ...
  return {
    ...
    binds_empty: !host.Binds || host.Binds.length === 0,
...
if (contract.maximum_trials?.pairs !== 1 || contract.maximum_trials?.total_cells !== 2) throw new Error("Phase 3 trial bound changed.");
if (contract.fixed_controls?.condition_order?.join(",") !== "candidate,baseline") throw new Error("Phase 3 condition order changed.");
if (contract.detector?.action?.command_field_examined !== false) throw new Error("Phase 3 action detector semantics changed.");
if (fs.existsSync(resultRoot) && walkFiles(resultRoot).some((file) => file.endsWith("manifest.json"))) throw new Error("The single Phase 3 pair already has authority; another run is prohibited.");
const dirty = execFileSync(git, ["status", "--porcelain"], { cwd: repoRoot, encoding: "utf8" }).trim();
if (dirty) throw new Error("Worktree must be clean; commit and test the Phase 3 contract, detector, fixtures, tests, runner, and validator before provider execution.");

const executionCommit = execFileSync(git, ["rev-parse", "HEAD"], { cwd: repoRoot, encoding: "utf8" }).trim();
const contractCommit = execFileSync(git, ["log", "-1", "--format=%H", "--", path.relative(repoRoot, contractFile)], { cwd: repoRoot, encoding: "utf8" }).trim();
const detectorCommit = execFileSync(git, ["log", "-1", "--format=%H", "--", path.relative(repoRoot, detectorFile)], { cwd: repoRoot, encoding: "utf8" }).trim();
for (const ancestor of [contractCommit, detectorCommit]) execFileSync(git, ["merge-base", "--is-ancestor", ancestor, executionCommit], { cwd: repoRoot, stdio: "ignore" });
if (contractCommit === detectorCommit || contractCommit === executionCommit || detectorCommit === executionCommit) {
  throw new Error("Provider pair requires separate contract, detector/test, and post-detector runner commits.");
}

const image = JSON.parse(execFileSync(docker, ["image", "inspect", contract.fixed_controls.runtime_image_id], { encoding: "utf8" }))[0];
if (image.Id !== contract.fixed_controls.runtime_image_id || image.Os !== "linux" || image.Architecture !== "amd64") throw new Error("Fixed Phase 3 runtime image is unavailable or has the wrong platform.");
...
const derivedSkillBytes = Buffer.concat([baseSkillBytes, Buffer.from(contract.candidate_test_copy.exact_append_utf8)]);
...
if (sha256(derivedSkillBytes) !== contract.candidate_test_copy.derived_skill_file_sha256) throw new Error("Derived Phase 3 SKILL.md hash mismatch.");
if (fixtureTree.sha256 !== contract.fixed_controls.fixture_tree_sha256) throw new Error("Phase 3 fixture changed after contract freeze.");
if (sha256(contract.fixed_controls.prompt) !== contract.fixed_controls.prompt_sha256) throw new Error("Phase 3 prompt hash mismatch.");
...
const runId = `${timestamp.toISOString().replace(/[-:TZ.]/g, "").slice(0, 14).toLowerCase()}-h1-structured-source-then-action`;
const localRoot = path.join(repoRoot, ".eval-work", "phase3", runId);
...
const createArgs = [
  ...
  "--env", "CODEX_HOME=/codex-home", "--env", "HOME=/home/phase3", "--env", "USERPROFILE=/home/phase3",
...
for (const condition of contract.fixed_controls.condition_order) {
  ...
  try {
    ...
    const inventory = walkFiles(path.join(workspace, ".agents", "skills", "goal-draft-policy"));
...
for (const condition of contract.fixed_controls.condition_order) {
  ...
  const candidateDetection = condition === "candidate" ? detectCandidateStructuredAction({
    events,
    derivedSkillPath: "/workspace/.agents/skills/goal-draft-policy/SKILL.md",
    derivedSkillText: derivedSkillBytes.toString("utf8"),
    marker: contract.detector.action.marker
  }) : null;
  const baselineDetection = condition === "baseline" ? detectBaselineAbsence({
    events,
    inventory: cell?.inventory || [],
    derivedSkillPath: "/workspace/.agents/skills/goal-draft-policy/SKILL.md",
    derivedSkillText: derivedSkillBytes.toString("utf8"),
    baseSkillText: baseSkillBytes.toString("utf8"),
    marker: contract.detector.action.marker
  }) : null;
  const result = {
    schema_version: "1.0-phase3-structured-action-witness-result",
    run_id: runId,
    ...
    baseline_detection: baselineDetection,
...
const candidateWitness = normalPair && candidate.candidate_detection.pass;
const baselineAbsence = normalPair && baseline.baseline_detection.pass;
const boundaryPass = prepared.length === 2 && prepared.every((item) => item.preflight.pass)
  && prepared.every((item) => item.preflight.mounts_empty && item.preflight.binds_empty
    && !item.preflight.docker_socket_mounted && !item.preflight.host_profile_mounted
    && !item.preflight.host_repository_mounted && !item.preflight.arbitrary_host_directory_mounted);
const expectedCandidateInventory = sourceTree.files.map((record) => record.path);
...
const freshPostFreezePair = contractCommit !== detectorCommit && detectorCommit !== executionCommit && contractCommit !== executionCommit;
...
const artifactHashValidation = publishedResults.every((entry) => {
  const resultFile = path.join(repoRoot, entry.result_path);
  const sidecarFile = path.join(repoRoot, entry.result_sidecar_path);
  return sha256(fs.readFileSync(resultFile)) === entry.result_sha256
    && fs.readFileSync(sidecarFile, "utf8").split(/\s+/)[0] === entry.result_sha256;
});
const publicSafetyValidation = resultObjects.every((result) => publicSafe(stableJson(result)));
const gates = {
  candidate_structured_action_witness: candidateWitness,
  ...
  fresh_post_contract_and_detector_pair: freshPostFreezePair
};
const decision = Object.values(gates).every(Boolean) ? "ADOPT witness method" : normalPair ? "REJECT" : "INCONCLUSIVE";
const manifest = {
  schema_version: "1.0-phase3-structured-action-witness-manifest",
  phase: "final-remediation-phase3-structured-action-witness",
  run_id: runId,
  authority: "revision",
  hypothesis_id: contract.hypothesis.id,
  ...
  detector_sha256: sha256(fs.readFileSync(detectorFile)),
  ...
  expected_action_output_utf8_bytes: contract.detector.action.expected_utf8_bytes,
  expected_action_output_sha256: contract.detector.action.expected_output_sha256,
  ...
  outer_boundary: {
    ...
    host_repository_mounted: false,
    arbitrary_host_directory_mounted: false,
    auth_transfer: "verified per-cell temp copy via docker cp; temp auth path removed after each cell",
  ...
  freshness: {
    contract_detector_execution_commits_distinct: freshPostFreezePair,
    contract_and_detector_ancestors_of_execution: true,
    pass: freshPostFreezePair
  },
  gates,
  normal_pair: normalPair,
  decision,
  interpretation: decision === "ADOPT witness method"
    ? "Only the isolation witness method is adopted; no Skill-quality or paired-comparison superiority claim is made."
    : "No witness method is adopted; further isolation remediation is prohibited and the bounded evidence program proceeds to closeout selection.",
  next_phase: decision === "ADOPT witness method" ? {
    entry: "valid paired smoke eligible but not started",
    maximum_pairs: 2,
    maximum_cells: 4,
    repetitions_per_cell: 1,
    retry: false,
    stop: "Predeclare a separate contract and stop after two pairs/four cells or the first infrastructure-invalid pair."
  } : {
    entry: "paired smoke ineligible",
    isolation_remediation: "closed; no retry, replacement, or additional hypothesis",
    closeout_candidate: "bounded REJECT/INCONCLUSIVE evidence-program closeout"
  },
  results: publishedResults,
  notes: "Raw JSONL, raw final output, stderr, auth bytes/hash, host paths, commands, and item/container/session identifiers remain local-only."
};
if (!publicSafe(stableJson(manifest))) throw new Error("Phase 3 manifest failed the local public-safety check.");
const manifestFile = path.join(publishRoot, "manifest.json");
writeHashedJson(manifestFile, manifest);
if (walkFiles(publishRoot).some((name) => /(?:trace\.jsonl|stderr\.txt|auth\.json|last-message\.txt)$/i.test(name))) throw new Error("Local-only Phase 3 artifact entered the public result directory.");

console.log(`Phase 3: ${decision}; ${path.relative(repoRoot, manifestFile)}`);
